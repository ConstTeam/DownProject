...
public final class CodeKit {
	...
	/* static fields */
	/** 库信息 */
	public static final String toString = "slib.util.CodeKit@v1.00.webgame";
	...
	public static short getCrc16(byte[] bytes, int pos, int len) {
		int crc = 0xFFFF;
	...
	/** 用指定crc获得16位CRC字节数组 */
	public static byte[] crc16(int crc) {
	...
	public static byte[] crc16(byte[] bytes) {
		short crc = getCrc16(bytes, 0, bytes.length);
		byte[] temp = new byte[bytes.length + 2];
		System.arraycopy(bytes, 0, temp, 0, bytes.length);
		temp[bytes.length] = (byte) (crc & 0xFF);
		temp[bytes.length + 1] = (byte) ((crc >>> 8) & 0xFF);
		return temp;
	}
	...
	/** 获得指定字节数组中指定位置和长度部分的32位CRC */
	public static int getCrc32_1(byte[] bytes, int pos, int len) {
		int crc = 0xFFFFFFFF;
		for (int i = pos; i < len; i++)
			crc = (crc >>> 8) ^ CRC_TABLE_32[(crc ^ bytes[i]) & 0xFF];
		return crc;
	...
	/** 获得指定字节数组中指定位置和长度部分的32位CRC */
	public static int getCrc32_2(byte[] bytes, int pos, int len, int crc) {
		for (int i = pos; i < len; i++)
			crc = (crc >>> 8) ^ CRC_TABLE_32[(crc ^ bytes[i]) & 0xFF];
		return crc;
	}
	...
	/** 获得指定字节数组中指定位置和长度部分的32位CRC */
	public static int getCrc32_3(byte[] bytes, int pos, int len, int crc) {
		for (int i = pos; i < len; i++)
			crc = (crc >>> 8) ^ CRC_TABLE_32[(crc ^ bytes[i]) & 0xFF];
		return ~crc;
	}
	...
	/** 校验指定字节数组中指定位置和长度部分的32位CRC */
	public static boolean isCrc32_3(byte[] bytes, int pos, int len, int crc) {
		for (int i = pos; i < len; i++)
			crc = (crc >>> 8) ^ CRC_TABLE_32[(crc ^ bytes[i]) & 0xFF];
		return crc == 0xDEBB20E3;
	}
	...
	/** 用指定crc获得32位CRC字节数组 */
	public static byte[] crc32(int crc) {
		byte[] temp = new byte[4];
		temp[0] = (byte) (crc & 0xFF);
		temp[1] = (byte) ((crc >>> 8) & 0xFF);
		temp[2] = (byte) ((crc >>> 16) & 0xFF);
		temp[3] = (byte) ((crc >>> 24) & 0xFF);
		return temp;
	}

	/** 给指定字节数组后加上32位CRC */
	public static byte[] crc32(byte[] bytes) {
		int crc = getCrc32(bytes, 0, bytes.length);
		byte[] temp = new byte[bytes.length + 4];
		System.arraycopy(bytes, 0, temp, 0, bytes.length);
		temp[bytes.length] = (byte) (crc & 0xFF);
		temp[bytes.length + 1] = (byte) ((crc >>> 8) & 0xFF);
		temp[bytes.length + 2] = (byte) ((crc >>> 16) & 0xFF);
		temp[bytes.length + 3] = (byte) ((crc >>> 24) & 0xFF);
		return temp;
	}

	/** 对指定字符串加密 */
	public static String codeing(String str, String key) {
		byte[] bytes = str.getBytes();
		CodeKit.coding(bytes, key);
	...
	/** 对指定字符串解密 */
	public static String deCoding(String str, String key) {
		String temp = null;
		byte[] bytes = new byte[str.length() / 2];
		for (int i = 0, j = 0; i < str.length(); i += 2, j++) {
			temp = String.valueOf("0x" + str.charAt(i) + str.charAt(i + 1));
			bytes[j] = Byte.decode(temp);
		}
		CodeKit.coding(bytes, key);
	...
	/**
	 * 将一个字节数组中指定位置和长度的部分， 用指定的字节数组进行编码，为滚动编码
	 */
	...
	/**
	 * 将一个字节数组中指定位置和长度的部分， 用指定的字节数组的偏移位置进行编码，为滚动编码
	 */
	public static void coding(byte[] bytes, int pos, int len, byte[] code,
			int offset) {
